...
import { Orb } from "@/components/salux/Orb";
import type { OrbState } from "@/components/salux/Orb";
import { InputPill } from "@/components/salux/InputPill";
import { ResponseSheet } from "@/components/salux/ResponseSheet";
import { HelpModal } from "@/components/salux/HelpModal";
import { detectWF, detectDashboardType, dashboardModes, workflows, type WorkflowKey } from "@/lib/salux-workflows";
...
function Index() {
  ...
  const speech = useSpeechRecognition({
    ...
    onResult: (t) => {
      setText(t);
      setStatus({ text: "Transcrição concluída — revise e envie", tone: "ok" });
    },
    onError: (e) => setStatus({ text: `Erro de voz: ${e}`, tone: "err" }),
  });
  ...
  const submit = useCallback(async (override?: string) => {
    ...
    setStatus({ text: `${wf.tag} — enviando…`, tone: "loading" });
    try {
      ...
      setStatus({ text: `Erro: ${(err as Error).message}`, tone: "err" });
      setOrbState("idle");
    } finally {
      setLoading(false);
    }
  ...
  const handleDetach = useCallback(() => {
    ...
    localStorage.setItem("salux_panel", JSON.stringify({
      data: responseData,
      tag: responseTag,
      query: responseQuery,
  ...
  const greeting = sheetOpen ? null : (
    ...
    >
      <p className="text-[15px] text-slate-500 font-light tracking-wide">Olá</p>
      <h1 className="mt-2 text-[34px] sm:text-[42px] font-semibold leading-[1.1] tracking-tight text-slate-800">
        Como posso ajudar<br />você hoje?
      </h1>
    </motion.div>
  );
  ...
  return (
    ...
    >
      ...
      <header className="relative z-10 flex items-center justify-between px-5 sm:px-8 pt-5">
        <div className="text-[13px] font-semibold tracking-[0.2em] text-slate-600">SALUX</div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => voice.toggleMute()}
            aria-label={voice.muted ? "Ativar voz" : "Desativar voz"}
      ...
      <div className="fixed bottom-0 inset-x-0 z-20 px-4 pb-6 pt-4 bg-gradient-to-t from-blue-100/70 to-transparent">
        ...
        <InputPill
          ...
          loading={loading}
          placeholder={speech.listening ? "Ouvindo…" : "Pergunte qualquer coisa…"}
        />
      </div>
