...
/**
 * Renderiza a resposta dos workflows Salux de forma legível.
 * Suporta: WF-16 (dashboards), WF-07 (documentação), WF-08 (regulatório), WF-12 (conversacional)
 * Faz fallback inteligente para JSON cru se a estrutura não for reconhecida.
 */
    ...
    <div className="space-y-4">
      ...
      {sugestao && (
        <button className="w-full text-left rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 p-3.5 transition-colors">
          <div className="text-[11px] uppercase tracking-wider text-white/45 mb-1">Sugestão</div>
...
function DashboardView({ data }: { data: any }) {
  ...
  const titles: Record<string, string> = {
    ...
    REGULATORIO: "Dashboard Regulatório",
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {data.status_operacional && (
          <Kpi
            ...
            value={data.status_operacional === "CRITICO" ? "🔴 Crítico" : data.status_operacional}
            tone={data.status_operacional === "CRITICO" ? "danger" : "ok"}
          />
        )}
        {data.alertas_ativos !== undefined && (
          <Kpi label="Alertas ativos" value={data.alertas_ativos} sub={data.fonte} tone={data.alertas_ativos > 3 ? "danger" : data.alertas_ativos > 0 ? "warn" : "ok"} />
        )}
        {data.glosas?.criticas !== undefined && (
          <Kpi label="Glosas críticas" value={data.glosas.criticas} sub={`${data.glosas.total_contas ?? 0} contas`} tone="danger" />
        )}
        {data.glosas?.valor_total_em_risco && (
          <Kpi label="Valor em risco" value={data.glosas.valor_total_em_risco} sub="glosas" tone="warn" />
        )}
        {data.fila?.sla_estourado !== undefined && (
          <Kpi label="SLA estourado" value={data.fila.sla_estourado} sub={`${data.fila.total ?? 0} na fila`} tone={data.fila.sla_estourado > 0 ? "danger" : "ok"} />
        )}
        {data.documentacao?.score_completude !== undefined && (
          <Kpi label="Completude" value={`${data.documentacao.score_completude}%`} sub={data.documentacao.status_faturamento} tone={data.documentacao.score_completude >= 80 ? "ok" : data.documentacao.score_completude >= 60 ? "warn" : "danger"} />
        )}
        {data.no_show?.alto_risco !== undefined && (
          <Kpi label="Alto risco no-show" value={data.no_show.alto_risco} sub={`${data.no_show.total_consultas ?? 0} consultas`} tone="warn" />
        )}
      </div>
      ...
      {data.documentacao?.documentos_faltantes?.length > 0 && (
        <div>
          <SectionTitle>Documentos faltantes</SectionTitle>
      ...
      {Array.isArray(data.top_insights) && data.top_insights.length > 0 && (
        <div>
          <SectionTitle>Insights prioritários</SectionTitle>
      ...
      {Array.isArray(data.unidades_criticas) && data.unidades_criticas.length > 0 && (
        <div>
          <SectionTitle>Unidades críticas</SectionTitle>
      ...
      {Array.isArray(data.glosas?.top_risco) && data.glosas.top_risco.length > 0 && (
        <div>
          <SectionTitle>Contas com maior risco de glosa</SectionTitle>
          <div className="space-y-1.5">
            {data.glosas.top_risco.slice(0, 6).map((c: any, i: number) => (
              <ScoreRow key={i} label={c.conta || `Conta ${i + 1}`} score={c.score} badge={c.acao} />
            ))}
          </div>
        </div>
      )}
      ...
      {Array.isArray(data.fila?.top_prioritarios) && data.fila.top_prioritarios.length > 0 && (
        <div>
          <SectionTitle>Pacientes prioritários</SectionTitle>
          <div className="space-y-1.5">
      ...
      {data.gargalos && (
        <div>
          <SectionTitle>Previsões de gargalo</SectionTitle>
          <div className="grid grid-cols-3 gap-3 mb-3">
            {data.gargalos.previsoes && (
              <>
                <Card>
                  <div className="text-[10px] uppercase tracking-wider text-white/50">Pacientes na fila</div>
                  <div className="text-2xl font-bold text-white">{data.gargalos.previsoes.pacientes_espera}</div>
                </Card>
                <Card>
                  <div className="text-[10px] uppercase tracking-wider text-white/50">Tempo médio espera</div>
                  <div className="text-2xl font-bold text-white">{data.gargalos.previsoes.tempo_medio_espera_min}min</div>
                </Card>
                <Card>
                  <div className="text-[10px] uppercase tracking-wider text-white/50">Ocupação</div>
      ...
      {Array.isArray(data.monitoramento?.unidades) && data.monitoramento.unidades.length > 0 && (
        <div>
          <SectionTitle>Status das unidades</SectionTitle>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {data.monitoramento.unidades.map((u: any, i: number) => {
              const statusColor = u.status === "CRITICO" ? "border-red-400/30 bg-red-500/10"
                : u.status === "ATENCAO" ? "border-amber-400/30 bg-amber-500/10"
      ...
      {Array.isArray(data.no_show?.acoes_urgentes) && data.no_show.acoes_urgentes.length > 0 && (
        <div>
          <SectionTitle>Ações urgentes - no-show</SectionTitle>
          <div className="space-y-2">
      ...
      {data.centro_cirurgico && (
        <div>
          <SectionTitle>Centro cirúrgico</SectionTitle>
          <div className="grid grid-cols-2 gap-3">
            <Card>
              <div className="text-[10px] uppercase tracking-wider text-white/50">Cirurgias alocadas</div>
              <div className="text-2xl font-bold text-white">{data.centro_cirurgico.cirurgias_alocadas}</div>
            </Card>
            <Card>
              <div className="text-[10px] uppercase tracking-wider text-white/50">Não alocadas</div>
...
function ListRow({ title, status, desc }: { title: string; status?: string; desc?: string }) {
  ...
  const dot = /CRITICO|CRÍTICO/.test(s) ? "bg-red-400" : s.includes("ALERTA") ? "bg-amber-400" : "bg-emerald-400";
  return (
...
function AlertRow({ message, severity }: { message: string; severity: string }) {
  const colors = severity === "CRITICA" || severity === "ALTA"
    ? "border-red-400/30 bg-red-500/10 text-red-200"
    : "border-amber-400/30 bg-amber-500/10 text-amber-200";
  return (
    <div className={`flex gap-3 rounded-xl border p-3 ${colors}`}>
      <div className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${severity === "CRITICA" ? "bg-red-400" : "bg-amber-400"}`} />
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <h2 className="text-lg font-semibold text-white">Documentação clínica</h2>
        {data.qualidade_estimada && <QualityBadge q={data.qualidade_estimada} />}
      </div>

      {/* Texto de evolução sugerido (principal) */}
      ...
      {(ev.subjetivo || ev.objetivo || ev.avaliacao || ev.plano) && (
        <>
          {ev.subjetivo && <Block title="Subjetivo" text={ev.subjetivo} />}
          {ev.objetivo && <Block title="Objetivo" text={ev.objetivo} />}
          {ev.avaliacao && <Block title="Avaliação" text={ev.avaliacao} />}
          {ev.plano && <Block title="Plano" text={ev.plano} />}
        </>
      )}
      ...
      {data.campos_preenchidos?.length > 0 && (
        <div>
          <SectionTitle>Campos preenchidos</SectionTitle>
          <div className="flex flex-wrap gap-2">
      ...
      {data.campos_faltantes?.length > 0 && (
        <div>
          <SectionTitle>Campos a completar</SectionTitle>
          <div className="flex flex-wrap gap-2">
      ...
      {Array.isArray(data.cids_sugeridos) && data.cids_sugeridos.length > 0 && (
        <div>
          <SectionTitle>CIDs sugeridos</SectionTitle>
          <div className="flex flex-wrap gap-2">
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <h2 className="text-lg font-semibold text-white">Análise regulatória</h2>
      ...
      {data.justificativa_clinica && (
        <Card className="!bg-white/[0.07]">
          <SectionTitle>Justificativa clínica</SectionTitle>
      ...
      {data.argumentos_principais?.length > 0 && (
        <div>
          <SectionTitle>Argumentos principais</SectionTitle>
          <div className="space-y-2">
      ...
      {data.embasamento_cientifico?.length > 0 && (
        <div>
          <SectionTitle>Embasamento científico</SectionTitle>
          <div className="space-y-2">
      ...
      {/* Recomendação / análise regulatória */}
      {(data.analise_regulatoria || data.recomendacao || data.justificativa) && (
        <>
          {data.analise_regulatoria && <Block title="Análise" text={data.analise_regulatoria} />}
          {data.recomendacao && <Block title="Recomendação" text={data.recomendacao} />}
          {data.justificativa && !data.justificativa_clinica && <Block title="Justificativa" text={data.justificativa} />}
        </>
      )}
...
function ProbBadge({ p }: { p: string }) {
  ...
  const cfg = v.includes("alta") ? "bg-emerald-500/20 text-emerald-200 border-emerald-400/30"
    : v.includes("media") || v.includes("média") ? "bg-amber-500/20 text-amber-200 border-amber-400/30"
    : "bg-red-500/20 text-red-200 border-red-400/30";
  return <span className={`px-2.5 py-1 rounded-md border text-xs font-medium ${cfg}`}>Aprovação {p}</span>;
}
...
function RawJson({ data }: { data: any }) {
  return (
    <details className="rounded-xl bg-white/5 border border-white/10 p-4">
      <summary className="cursor-pointer text-xs uppercase tracking-wider text-white/55">Resposta JSON</summary>
