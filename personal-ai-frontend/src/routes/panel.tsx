...
import { ResponseRenderer } from "@/components/personal-ai/ResponseRenderer";
import { workflows, type WorkflowKey } from "@/lib/personal-ai-workflows";
...
function Panel() {
  ...
  useEffect(() => {
    const raw = localStorage.getItem("personal_ai_panel");
    if (!raw) return;
  ...
  if (!payload) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: "oklch(0.12 0.03 260)" }}>
        <p className="text-white/50 text-sm">No panel detached.</p>
      </div>
    );
  }
