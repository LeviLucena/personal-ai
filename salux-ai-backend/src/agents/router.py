...
router_prompt = ChatPromptTemplate.from_messages([
    ("system", "Classifique o comando do usuário em um dos seguintes tipos de dashboard: "
     "executivo, financeiro, regulatorio, operacional, agenda. "
     "Responda APENAS com o nome do tipo, sem explicação."),
    ("human", "{comando}"),
])
...
DASHBOARD_MAP = {
    "executivo": {
        ...
        "description": "Status do hospital, resumo geral",
    },
    "financeiro": {
        "workflows": ["predicao_glosa", "conferencia_documental"],
        "description": "Risco financeiro, glosas, faturamento",
    },
    "regulatorio": {
        "workflows": ["priorizacao_filas", "copilot_regulatorio"],
        "description": "Fila regulatória, autorização",
    },
    "operacional": {
        "workflows": ["gargalos", "monitoramento", "agendamento_cirurgico"],
        "description": "Gargalos, centro cirúrgico, capacidade",
    },
    "agenda": {
        "workflows": ["predicao_noshow", "agendamento_cirurgico"],
        "description": "Agendamento, consultas, no-show",
    },
}
...
async def synthesize_response(state: RouterState) -> dict:
    ...
    status_list = [u["status"] for u in unidades if u.get("status") == "CRITICO"]
    merged["status_operacional"] = "CRITICO" if status_list else "NORMAL"
    merged["alertas_ativos"] = len(status_list)
    merged["unidades_criticas"] = [
        u["nome"] for u in unidades if u.get("status") == "CRITICO"
    ]
    ...
    if garg:
        merged["gargalos"] = {
            ...
            "top_alertas": [{"mensagem": a, "severidade": "CRITICA"} for a in garg.get("acoes", [])],
        }
    ...
    if gli:
        ...
        merged["glosas"] = {
            "criticas": sum(1 for c in contas if c.get("classificacao") == "CRITICA"),
    ...
    if doc:
        merged["documentacao"] = {
            ...
            "status_faturamento": "APROVADO" if doc.get("score_completude", 0) >= 80 else "PENDENTE",
        }
    ...
    if fila:
        ...
        merged["fila"] = {
            "total": len(fp),
            "criticas": sum(1 for f in fp if f.get("prioridade") in ("CRITICA", "ALTA")),
    ...
    if noshow:
        ...
        merged["no_show"] = {
            "alto_risco": sum(1 for a in ag if a.get("risco") == "ALTA"),
    ...
    result = await conv_graph.ainvoke({
        "messages": [
            ("user", f"Resuma o seguinte dashboard {tipo} em até 3 frases curtas:\n{summary_input}")
        ]
    })
